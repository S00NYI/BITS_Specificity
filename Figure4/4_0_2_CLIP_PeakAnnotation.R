## CLIP Peak Annotation
## Original script by Dr. Joseph M. Luna.
## Edited by Soon Yi
## Last edit: 2025-08-09
...
## Load peak file:
################################################################################
Base_PATH = '~/Desktop/Genomics/Specificity/Sequencing/COLLAPSED/FilteredPeaks/'
peaksMatrix_FILEs = c('hnRNPC_WT', 'hnRNPC_Mut', 'RBM25_WT', 'RBM25_WT_old', 'RBM25_WT_new', 'RBM25_Mut', 'RBM25_Mut_old', 'RBM25_Mut_new', 'hnRNPC_WT_inRBM25_WT', 'hnRNPC_WT_inRBM25_Mut')
################################################################################

for (peaksMatrix_FILE in peaksMatrix_FILEs) {
  ## Read Peak file
  ################################################################################
  peaksMatrix_PATH = paste0(Base_PATH, peaksMatrix_FILE, '_Combined_peakCoverage_filtered.txt')
  
  peaksMatrix = read.delim(peaksMatrix_PATH, header = TRUE, sep = "\t")
  ...
  ## Priority list
  ################################################################################
  priority_list = c("intron",  
                    "3'UTR", "5'UTR", "CDS",
                    "tRNA", "rRNA", "miRNA", "snoRNA",  "scaRNA",  "snRNA", "lncRNA", "miscRNA", "TE",  "Other",
  ...
  peaksMatrix = peaksMatrix %>%
    mutate(grouped_annotation = ifelse(finalized_annotation %in% c("5'UTR", "CDS", "3'UTR", "intron", "tRNA", "rRNA", "TE", "Other", "unannotated"), finalized_annotation,
                                       ifelse(finalized_annotation %in% c("CDS_Retained_intron", "ncRNA_Retained_intron"), "Retained_intron",
                                              ifelse(finalized_annotation %in% c("miRNA", "lncRNA", "scaRNA", "snRNA", "miscRNA"), "ncRNA", "unannotated"))),
           annotation_count = sapply(strsplit(annotation, "\\|"), length))  # Count elements after splitting
  ################################################################################
  
  ## Finalize annotation and write to file
  ################################################################################
  ...
  write.table(apply(peaksMatrix,2,as.character), paste0(str_replace(str_replace(peaksMatrix_PATH, "FilteredPeaks", "AnnotatedPeaks"), '.txt', '_annotated.txt')), 
