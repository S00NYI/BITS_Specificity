...
################################################################################
baseDir = '~/Desktop/Genomics/Specificity/Sequencing/IndividualPeaks/Final'
outputDir = '~/Desktop/Genomics/Specificity/AnalysisOutput/OLD_SCRIPT_VERSION/'
...
for (sampleNAME in sampleNAMEs) {
  ...
  for (extension in extensions) {
    ...
    TOP_MOTIF = c(TOP_MOTIF, KMer_Matches_Scaled$MOTIF[KMer_Matches_Scaled$COUNT == max(KMer_Matches_Scaled$COUNT)])
    
    UUUUU_IS = c(UUUUU_IS, as.numeric(formatC(KMer_SpecificityIndex$COUNT[KMer_SpecificityIndex$MOTIF=='TTTTT'], digits = 2, format = "f")))
    GGGGG_IS = c(GGGGG_IS, as.numeric(formatC(KMer_SpecificityIndex$COUNT[KMer_SpecificityIndex$MOTIF=='GGGGG'], digits = 2, format = "f")))
    
    ordered_IS = KMer_SpecificityIndex[order(-KMer_SpecificityIndex$COUNT), ]
    ...
    write.csv(Peak_KMers_C,
              paste0(outputDir, DATE, '/', sampleNAME, '_RelativeFrequency_', extension, 'ntExt_5mer.csv'), quote = F)
    write.csv(KMer_Matches_Scaled,
              paste0(outputDir, DATE, '/', sampleNAME, '_RelativeFrequencyNormalized_', extension, 'ntExt_5mer.csv'), quote = F)
    write.csv(KMer_SpecificityIndex,
              paste0(outputDir, DATE, '/', sampleNAME, '_SIperMotif_', extension, 'ntExt_5mer.csv'), quote = F)
    write.csv(hist_count,
              paste0(outputDir, DATE, '/', sampleNAME, '_Histogram_', extension, 'ntExt_5mer.csv'), quote = F)
  }
}
...
write.csv(TopMotif_Data_Compiled,
          paste0(outputDir, DATE, '/TopMotif_SI_5mer_Optimization.csv'), quote = F)
